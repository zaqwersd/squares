...
const JUMP_DIRECTION_VARIANCE := 0.42
...
func configure(new_target: IslandPlayer, spawn_seed: int) -> void:
	target = new_target
...
func _begin_jump() -> void:
	...
	_jump_direction = toward_player.rotated(_movement_rng.randf_range(-JUMP_DIRECTION_VARIANCE, JUMP_DIRECTION_VARIANCE))
	if island_map != null:
...
func _update_jump(delta: float) -> void:
	...
	var can_damage_target := (
		absf(target.global_position.x - global_position.x) <= LANDING_DAMAGE_HALF_EXTENT
		and absf(target.global_position.y - global_position.y) <= LANDING_DAMAGE_HALF_EXTENT
	)
	if not can_damage_target and global_position.distance_to(previous_position) < movement.length() * 0.2:
		var island_map := get_parent() as IslandMap
		if island_map != null:
			_jump_direction = island_map.get_unblocked_movement_direction(self, _jump_direction, movement_speed, delta)
		else:
			_jump_direction = _jump_direction.orthogonal()
